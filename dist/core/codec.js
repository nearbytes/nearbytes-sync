...
function wrapLengthPrefixed(body) {
...
export function encodeBlockStreamBegin(hashHex, total) {
...
function encodeEventData(message) {
...
export function createWireDecoder(handlers) {
    let frameBuf = new Uint8Array(0);
    let streamTotal = 0;
    let streamReceived = 0;
    const appendFrames = (chunk) => {
    ...
    const decodeControl = (body) => {
        const json = body.subarray(1);
        return JSON.parse(new TextDecoder().decode(json));
    };
    const decodeEvent = (body) => {
    ...
    const parseFrames = () => {
        while (frameBuf.length >= 4) {
            const size = new DataView(frameBuf.buffer, frameBuf.byteOffset, frameBuf.byteLength).getUint32(0, false);
